//
//  RestaurantPlaceMarkAnnotationView.m
//  TasteMakers
//
...
/*
 I stole a lot from this github. https://github.com/govindaraokondala/Custom-Map-Annotation-Callouts-for-IOS/blob/master/CustomMapAnnotationsCalloutsforIOS/CustomMapLibraryClasses/CalloutMapAnnotationView.m
 Afterwards I tweaked it a lot to have it how I wanted it
*/

#import "RestaurantPlaceMarkAnnotationView.h"
...
#define CalloutMapAnnotationViewHeightAboveParent 3.0f

@interface RestaurantPlaceMarkAnnotationView()
...
@implementation RestaurantPlaceMarkAnnotationView
...
- (void)adjustMapRegionIfNeeded {
    ...
    //     NSLog(@"Rect2: %f %f %f %f ",self.frame.origin.x,self.frame.origin.y,self.frame.size.width,self.frame.size.height);
}
...
- (void)drawRect:(CGRect)rect {
    //self.frame=CGRectMake(self.bounds.origin.x, self.bounds.origin.y, 320, self.bounds.size.height);
    CGFloat stroke = 1.0;
    CGFloat radius = 7.0;
    ...
    if([self relativeParentXPosition]>=300)
    {
        rect.origin.x+=parentX-300+20;
        UIView *v=(UIView *)[[self.contentView subviews] objectAtIndex:0];
